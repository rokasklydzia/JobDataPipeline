...
# Function to store jobs in the database using ON CONFLICT
def store_reed_jobs(**kwargs):
    ...
    # Insert or update job ads into the database
    for job_ad in mapped_job_ads:
        stmt = insert(JobAd).values(
            id=job_ad.id,
            job_title=job_ad.job_title,
            company_name=job_ad.company_name,
            job_url=job_ad.job_url,
            job_type=job_ad.job_type,
            region=job_ad.region,
            salary=job_ad.salary,
            timestamp=job_ad.timestamp
        ).on_conflict_do_update(
            index_elements=['id'],  # Use 'id' to detect conflicts
            set_={
                'job_title': job_ad.job_title,
                'company_name': job_ad.company_name,
                'job_url': job_ad.job_url,
    ...
    print(f"Successfully inserted/updated {len(mapped_job_ads)} job ads into the database.")
